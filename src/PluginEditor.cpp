...
namespace BeatCrafter
{
	...
	void BeatCrafterEditor::setupComponents()
	{
		...
		tripletModeMidiLearnButton.onClick = [this]()
			{ onTripletModeMidiLearnClicked(); };
	...
	void BeatCrafterEditor::paint(juce::Graphics& g)
	{
		...
		g.setFont(titleFont);
		g.drawText("BEATCRAFTER", 10, 8, 250, 35, juce::Justification::left);

		g.setColour(modernLookAndFeel.textDimmed);
		g.setFont(juce::Font(10.0f));
		g.drawText("v0.1.1", 230, 18, 50, 20, juce::Justification::left);
	}
	...
	void BeatCrafterEditor::resized()
	{
		...
		auto slotAreaWithMidi = bounds.removeFromBottom(80);
		auto slotRow = slotAreaWithMidi.removeFromTop(30).reduced(10, 0);
		slotManager->setBounds(slotRow.reduced(2, 0));

		auto slotMidiArea = slotAreaWithMidi.removeFromBottom(40);
		...
		auto rightColumn = contentArea.removeFromRight(200);
		...
		for (int i = 0; i < 8; ++i)
		{
			updateMidiLearnPair(
				*slotMidiLearnButtons[i], slotMidiLabels[i],
				processor.isMidiLearning() && processor.getMidiLearnTarget() >= 1 && processor.getMidiLearnSlot() == i,
